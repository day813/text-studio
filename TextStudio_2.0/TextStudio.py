...
REMOTE_HISTORY_PATH = Path.home() / "textstudio" / "remote_connections.json"
REMOTE_HISTORY_LIMIT = 20
LOCAL_DOWNLOAD_FOLDER = "textlibrary"


def local_textstudio_download_dir():
...
class LazyLineIndex:
    ...
    def __init__(self, path: str, size: int, mtime_ns: int, newline: bytes, bom: int = 0):
        ...
        self.cache_dir = Path.home() / "textstudio" / "indexes"
    ...
    def start(self):
        with self.lock:
            ...
            self.generation += 1
            gen = self.generation
            self.thread = threading.Thread(target=self._worker, args=(gen,), daemon=True, name="TextStudioLineIndex")
            self.thread.start()
...
class AnnotationStore:
    ...
    def _migrate_legacy_schema(self):
        """Add missing additive fields used by older TextStudio databases."""
...
class DocumentView(ttk.Frame):
    ...
    def goto_line(self, line: int):
        if getattr(self.backend, "is_remote", False):
            ...
            threading.Thread(target=worker, daemon=True, name="TextStudioRemoteLineJump").start()
    ...
    def find_next(self, backwards=False, pattern=None, regex_mode=None, case_sensitive=None):
        ...
        threading.Thread(target=worker, daemon=True, name="TextStudioSearch").start()
    ...
    def _start_remote_live_poll(self):
        ...
        threading.Thread(
            target=worker, daemon=True, name="TextStudioRemoteLiveTail"
        ).start()
    ...
    def export_data(self):
        default = os.path.basename(getattr(self.backend, "path", self.path)) + ".textstudio.json"
...
class RemoteBrowserDialog(tk.Toplevel):
    ...
    def __init__(self, app):
        ...
        # opened remote tabs remain owned by TextStudio until the app exits.
    ...
    def _start_task(self, kind, func):
        ...
        threading.Thread(target=worker, daemon=True, name=f"TextStudioRemote-{kind}").start()
    ...
    def _start_remote_dir_search(self):
        ...
        threading.Thread(
            target=worker, daemon=True, name="TextStudioRemoteDirectorySearch"
        ).start()
    ...
    def _download_selected(self):
        ...
        if row["type"] not in ("文件", "目录", "链接"):
            ...
            return
        full = row.get("path") or posixpath.join(self.current_dir, row["name"])
        local_root = local_textstudio_download_dir()
        try:
        ...
        threading.Thread(
            target=worker, daemon=True, name="TextStudioBackgroundDownload"
        ).start()
    ...
    def _upload_files(self):
        ...
        threading.Thread(
            target=worker, daemon=True, name="TextStudioBackgroundUpload"
        ).start()
...
def main():
    ...
    if "-h" in args or "--help" in args:
        print(
            f"TextStudio {VERSION}\n"
            "Usage: python3 TextStudio.py [file1.log file2.txt ...]\n"
